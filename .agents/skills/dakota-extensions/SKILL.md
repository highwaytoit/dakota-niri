...
- **Unprivileged System State**: Extensions run inside the user's GNOME Shell process and have no root, so system state must come from an interface a session can actually read: systemd unit state on the system bus, a world-readable file, or a D-Bus service. Privileged subprocess probes fail inside a `catch` and leave the feature permanently inert with no log. For deployment/reboot state specifically, see the staged-deployment invariant in `dakota-image`; `bootc status` is not callable from a session.
